...
使用法线贴图计算光照的着色器与在 [Lighting Maps](Ch%2013%20Lighting%20Maps.md) 中使用的着色器并无太大区别，只不过之前每个片元法线的值，是在顶点着色器中根据顶点的法线计算，然后再插值得到。而现在片元的法线值是通过法线贴图求得，需要注意 RGB 通道的范围是 $[0,1]$ 而法线每个分量的范围是 $[-1,1]$ ，因此还需要进行转换。因此片段着色器中需要进行的改变如下所示：
