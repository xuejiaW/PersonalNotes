...
接口块（Interface Block）与在 [LearnOpenGL-Ch 12 Materials](LearnOpenGL-Ch%2012%20Materials.md) 使用的 `Struct` 有点类似，但它是为了让着色器之间传递的变量（即通过 `in` 和 `out` 传递的变量）可以组合在一起，因此必须通过 `in` 和 `out` 修饰，而不能在uniform使用。
...
![](assets/LearnOpenGL-Ch%2021%20Advanced%20Data,%20GLSL/Untitled.png)
...
# 结果与源码：
![](assets/LearnOpenGL-Ch%2021%20Advanced%20Data,%20GLSL/AdvancedDataGLSL.gif)
